...
def chat():
    ...
    # الحصول على مزود الذكاء الاصطناعي المحدد أو استخدام واحد عشوائي (التفاصيل تدار في ai_assistant.py)
    provider = data.get('provider', None)  # لا يتم عرضه للمستخدم
    
    try:
        # الحصول على استجابة من نموذج الذكاء الاصطناعي باستخدام الوحدة الخارجية
        answer = get_ai_response(question, provider)
